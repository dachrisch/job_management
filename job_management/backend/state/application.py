...
class ApplicationState(rx.State):
    ...
    def load_current_job_offer(self):
        ...
        if job_url:
            ...
            self.analyzed_job_offer = self.application_service.load_job_analysis(self.job_offer)
    ...
    async def compose_application(self):
        ...
        application = self.application_service.compose_application(self.job_offer)

        async with self:
            self.load_current_job_offer()
            self.job_offer.state.is_composing = False
            self.job_offer.state.composed = True
